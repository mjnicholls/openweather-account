...
import { Alert, Button, Col, Row, Input, Label } from 'reactstrap'
...
const EditTrigger = ({ key, userId, id, name, status, close }) => {
  console.log('status', status)
  const [error, setError] = useState({})

  const [activeName, setActiveName] = useState(key ? key.name : '')
      ...
      >
       ...
       <Row className="search-box">
          <Col className="text-end">
              <Button className="button-neutral" onClick={refreshPage}>Back to all triggers</Button>
          </Col>
        </Row>
      </ReactBSAlert>,
    )
